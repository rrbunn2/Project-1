...
We can then make some comparisons between the effective power and the number of times an ability can be used by looking at a scatter plot
```{r scatter}
...
```{r berries}
berries <- API_Wrapper("berry",type="berry",limit=100)[,1]
...
```{r boxplot}
data2 <- data2 %>% mutate(g_class=if_else(growth >15,"fast",if_else(growth >10,"average",if_else(growth > 5,"slow","very slow"))),"effeciency"=harvest/growth)
...
j+geom_bar(aes(fill=g_class),position = "dodge")
```
...
```{r tables}
data2 %>% select(g_class,firmness) %>% table()
...
data2 %>% group_by(g_class) %>% summarise(avg_growth=mean(growth),avg_size=mean(size),avg_harvest=mean(harvest))
...
```{r pokemonData}
pokemons <- API_Wrapper("pokemon",type="pokemon",limit=100)[,1]
...
```{r}
j<-ggplot(data3,aes(type))
j+geom_bar()
```

Next we can create a scatterplot of height vs weight grouped by the pokemon type
```{r}
k <- ggplot(data3,aes(height,weight))
k+geom_point(aes(color=type))
```
